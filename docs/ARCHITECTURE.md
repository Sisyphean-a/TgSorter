...
- `AuthController`
  - 物理位置在 `lib/app/features/auth/application/auth_controller_legacy.dart`，旧路径仅保留导出壳。
  - 监听 TDLib 授权状态。
  - 提交手机号、验证码、二步验证密码。
  - 启动失败时将错误写入全局错误控制器。
- `PipelineController`
  - 物理位置在 `lib/app/features/pipeline/application/pipeline_controller_legacy.dart`，旧路径仅保留导出壳。
  - 管理当前消息、缓存页、上一条/下一条导航、批处理、撤销、重试队列。
  - 负责媒体刷新轮询逻辑。
  - 通过 `PipelineSettingsProvider` 读取当前生效配置，避免直接依赖 `SettingsController` 具体实现。
- `SettingsController`
  - 物理位置在 `lib/app/features/settings/application/settings_controller_legacy.dart`，旧路径仅保留导出壳。
  - 同时维护已保存配置与草稿配置。
  - 管理分类目标、来源会话、代理、快捷键、批处理参数等设置。
  - 提供统一 `saveDraft()` / `discardDraft()` 保存模型。
- `AuthCoordinator` / `PipelineCoordinator` / `SettingsCoordinator`
  - 作为新的 feature 协调层，承接 capability interface 与拆分后的子服务。
- `AppErrorController`
  - 统一收敛启动错误和运行期错误。
...
- 这类“路由装配 + 能力接口 + coordinator/service”边界，用于削减 `Get.find()` 和跨控制器强耦合。
...
lib/
  ...
  app/
    ...
    bindings.dart             # 兼容入口，转发到 core/di
    core/
      di/                     # 模块化依赖注入
      routing/                # 路由常量与页面注册
    controllers/              # 兼容导出壳
