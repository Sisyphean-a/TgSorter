...
| Local File Access    | path_provider                | `^2.1.5`      |
...
| Audio Backend Bridge | just_audio_media_kit         | `^2.1.0`      |
| Windows Video Libs   | media_kit_libs_windows_video | `^1.0.11`     |
| Android Video Libs   | media_kit_libs_android_video | `^1.3.8`      |

## Startup Flow

1. `lib/main.dart`
   - 初始化 Flutter Binding。
   - 通过 `DefaultPlaybackCapabilityService.detect()` 按平台初始化音频 / 视频播放能力。
   - 注册启动期错误写入 `startup_error.log`。
   - 启动 `BootstrapApp`。
2. `lib/app/bootstrap_app.dart`
   - 在 `initState()` 中触发传入的初始化回调。
   - `FutureBuilder` 只负责监听初始化结果。
   - 根据初始化状态展示 loading / error / app。
3. `lib/app/core/di/app_bindings.dart`
   - `main.dart` 把 `registerAppBindings` 作为回调传给 `BootstrapApp`。
   - 创建 `SharedPreferences` 仓储、TDLib 传输层、适配器与全局错误控制器。
   - 分模块注册 `settings / auth / pipeline` 依赖。
4. `lib/app/app.dart`
   - 启动 `GetMaterialApp`。
   - 通过 `lib/app/core/routing/app_routes.dart` 注册 `/auth` 与 `/app`。
   - `/app` 路由承载 `MainShellPage`，在同一壳层内切换工作台、设置和日志三个目标页面。
...
- `lib/app/features/auth/presentation/auth_page.dart`
  - 登录页面。
- `lib/app/features/shell/presentation/main_shell_page.dart`
  - 主应用壳层，负责抽屉导航以及工作台 / 设置 / 日志三块内容切换。
- `lib/app/features/pipeline/presentation/pipeline_page.dart`
  - 工作台内容入口，组合错误面板与移动端/桌面端布局。
...
- `lib/app/features/settings/presentation/settings_screen.dart`
  - 设置模块真实入口，负责首页 / 详情页切换、页面草稿态、顶部保存动作以及 Telegram 风格设置容器组合。
- `lib/app/features/settings/presentation/settings_surface.dart`
  - 设置页滚动表面与桌面端窄列容器。
- `lib/app/features/settings/presentation/settings_telegram_tiles.dart`
  - 设置页专用 Telegram 风格列表行、分组块和开关行。
- `lib/app/features/settings/presentation/logs_screen.dart`
  - 日志页面内容，按分类链路查看失败、重试和最终结果。
...
- `SettingsCoordinator`
  ...
  - 实现 `AuthSettingsPort` 与 `PipelineSettingsReader`，分别向登录流和流水线暴露最小配置契约。
- `AppErrorController`
  - 定义在 `lib/app/shared/errors/app_error_controller.dart`，统一收敛启动错误和运行期错误。
...
- feature 间能力边界统一定义在 `features/*/ports/*`，不再通过 `application/*_gateway.dart` 过渡壳转发。
- `features/shell` 作为顶层组合层，只负责把 pipeline、settings 和 logs 组合进单一应用外壳，不承载业务端口实现。
- 跨 feature 的可复用展示组件与格式化器统一沉淀到 `shared/presentation/*`。
- `TelegramService` 直接以 capability-based facade 形式实现鉴权、读取、媒体、分类、恢复和会话查询 ports。
- `SettingsCoordinator` 通过 `AuthSettingsPort` 与 `PipelineSettingsReader` 向其他 feature 暴露配置契约。
- `PipelineCoordinator` 通过 `PipelineLogsPort` 向设置页暴露最近操作日志，而不是让设置页直接依赖流水线协调器具体实现。
...
1. 媒体播放能力依赖平台：当前内联视频主要针对 Android / Windows，内联音频支持 Android / Windows / Linux。
2. 第三方客户端中的最终删除显示样式无法完全由本项目控制。
3. 启动期错误会追加写入 `startup_error.log`，但仍以本地文件记录为主，没有额外的远端错误收集链路。
...
- 音频 / 视频播放后端会在启动时根据平台能力初始化，桌面侧依赖 media_kit 相关动态库。
...
- cross-feature port 装配与模块化 DI 收口。
